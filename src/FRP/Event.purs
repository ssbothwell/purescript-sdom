...
import Data.Either (Either(..), either, hush, fromLeft', fromRight', isLeft, isRight)
...
instance compactableEvent :: Compactable Event where
  compact event = map (\x -> unsafePartial fromJust x) (filter isJust event)
  separate event =
    { left: unsafePartial (map fromLeft) (filter isLeft event)
    , right: unsafePartial (map fromRight) (filter isRight event)
    }
    where
      fromLeft = fromLeft' (\_ -> unsafeCrashWith "Unexpected Right")
      fromRight = fromRight' (\_ -> unsafeCrashWith "Unexpected Left")
